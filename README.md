...
Open manual vital entry and tap the camera icon. Grant camera access, then fit the numbers inside the three guides in top-to-bottom order: systolic blood pressure, diastolic blood pressure, and pulse. The default proportions accommodate Microlife displays with a smaller pulse row. Drag the four corners to surround the numeric column, and drag the two divider handles into the gaps between rows. The left/right handles in each row independently adjust its width, allowing a tighter pulse region without excluding a leading blood-pressure digit. Keep a small margin around every digit; keep LCD borders, labels, dates, times, and memory numbers outside the guides. Tap `読み取る` and hold the phone still while focus and exposure settle. Compare the frozen image and all three recognized values, then tap `入力欄に反映`. Review or correct the values and tap `Health Connectに登録` as usual. The measurement time remains the time of registration.

Recognition uses CameraX `ImageCapture` in quality mode with flash disabled, rather than a preview screenshot. Preview and capture share a `ViewPort`; the captured image's crop rectangle and rotation are applied before mapping the guides. The selected four corners correct perspective, then the two dividers split the image into three individual rows. Both OCR and the seven-segment reader process each entire row separately, so a smaller pulse is no longer downscaled together with the whole display. A focus failure or timeout requires a retry. Cameras without region metering support still use the normal ImageCapture auto-exposure/focus pipeline.

The capture rotation follows display changes, including a 180-degree turn that does not recreate the activity or resize the preview. It is refreshed on resume and immediately before capture as well.
...
Text recognition ignores only known labels and units (such as `SYS`, `DIA`, `PUL`, and `mmHg`), including trailing periods such as `SYS.` and `/min.`. Unknown text and split digits are not repaired or joined as text; the pixel reader must independently recognize the complete number to recover them. Recognized decimal points, signs, dates, and times block that fallback. A stroke touching a row boundary requires reframing; the app does not trim edges or discard a possible leading digit. All three rows must be recognized, their engines must not conflict, and the combined values must pass the existing registration-input checks.

Each framed row first normalizes horizontal lighting bands, then compares local contrast levels. The spacing and background estimates account for the smaller crop, including sparse `1`/`7` strokes. If no reading is detected, a horizontal background estimate is tried. This retry cannot override an uncertain first pass and still requires agreement across multiple contrast levels. The original whole-column reader remains covered by the existing regression tests.
...
Row-background stroke evidence is also retained before local averaging, which can otherwise hide a faint leading digit beside darker digits. An unresolved dense vertical stroke beside or between the decoded digits requires a retry. Regression tests cover a partially missing, faded leading `1` in `180 / 60 / 65`, preventing acceptance as `80 / 60 / 65` even when OCR misses that stroke.
...
The individual-row regression also covers the screenshot's three values with a narrower pulse crop and rejection of the ambiguous photographed systolic row. Photo fixtures verify recognition of explicitly framed pixels; they do not exercise Android perspective correction, autofocus, exposure or ML Kit.

Known limitation: a sufficiently faint segment can disappear at every tested contrast level, so agreement across thresholds does not prove that the digits are correct. A synthetic faded `9` can still become `3` in both the original column reader and the row reader. The camera result remains an editable input suggestion and must be compared with the physical display before registration.
...
Device verification: check the first read in airplane mode, camera permission denial and later grant, cancellation during focus/capture/OCR with existing input values, portrait/landscape rotation (including landscape-to-reverse-landscape 180-degree turns and rotation while the app is in the background), enlarged text, all four corner handles and both dividers, focus failure and retry, correspondence between live guides and the frozen photo, and actual monitor readings under different lighting and modest perspective angles. Compare successful correct reads, incorrect reads, and rejected reads separately. Returning from the camera must only fill the input fields; Health Connect registration still requires its registration button.
